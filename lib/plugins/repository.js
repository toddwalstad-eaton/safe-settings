...
module.exports = class Repository extends ErrorStash {
  ...
  async updateSecurity (repoData, resArray) {
    if (this.security?.enableVulnerabilityAlerts === true || this.security?.enableVulnerabilityAlerts === false) {
      ...
      // Check current state to avoid redundant API calls
      let alertsCurrentlyEnabled = false
      try {
        await this.github.rest.repos.checkVulnerabilityAlerts({
          owner: repoData.owner.login,
          repo: repoData.name
        })
        alertsCurrentlyEnabled = true
      } catch (e) {
        if (e.status === 404) {
          alertsCurrentlyEnabled = false
        } else {
          this.log.debug(`Error checking vulnerability alerts status: ${e.message}`)
          alertsCurrentlyEnabled = !this.security.enableVulnerabilityAlerts // assume out of sync
        }
      }

      if (this.security.enableVulnerabilityAlerts === true) {
        if (alertsCurrentlyEnabled) {
          this.log.debug(`Dependabot alerts already enabled for ${repoData.name}, skipping`)
          return Promise.resolve([])
        }
        ...
        if (!alertsCurrentlyEnabled) {
          this.log.debug(`Dependabot alerts already disabled for ${repoData.name}, skipping`)
          return Promise.resolve([])
        }
