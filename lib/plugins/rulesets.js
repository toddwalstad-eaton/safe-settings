...
module.exports = class Rulesets extends Diffable {
  ...
  changed (existing, attrs) {
    ...
    if (merged.hasChanges) {
      this.log.info(`[Rulesets] Diff detected for "${attrs.name}": additions=${JSON.stringify(merged.additions)}, modifications=${JSON.stringify(merged.modifications)}, deletions=${JSON.stringify(merged.deletions)}`)
      this.log.debug(`[Rulesets] existing=${JSON.stringify(existing)}`)
      this.log.debug(`[Rulesets] desired=${JSON.stringify(attrs)}`)
    }
